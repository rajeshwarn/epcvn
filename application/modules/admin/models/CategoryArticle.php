<?php
class Admin_Model_CategoryArticle extends  Zend_Db_Table_Abstract {
	protected $_name = 'category_article';
	protected $_primary = 'id';
	protected $_dependentTables = array('Admin_Model_Article');
	
	public function getCategoryArticleInfo( $param = null, $options = null) {
		if($options == null) {
			$select = $this->select()
							->where('isDelete = 0');
	...
	public function getItemInSelectbox($arrParam = null, $options = null) {
			...
			$select = $this->select()
			->from("{$this->_name}", array('id', 'titleVi', 'level', 'status', 'parentId', 'weight', 'createdBy'))
			->where('isDelete = 0')
			->order('weight ASC');
	...
	public function getCategoryArticles($params = null, $options = null) {
		if ($options == null) {
			$select = $this->select()
			->where('isDelete = 0');
		} elseif ($options['task'] == 'by-id') {
	...
	public function getCategoryArticleOne($id) {
		$select = $this->select()
		->where('id = ?', (int) $id)
		->where('isDelete = 0');
	 ...
	 public function removeItem($params = null, $options = null) {
	 	try {
	 		...
	 		if($options == null){
	 			...
	 			$params = $this->getCategoryArticles();
